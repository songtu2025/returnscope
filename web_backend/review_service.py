...
DERIVED_CLASSIFICATION_FIELDS = ("semantic_review",)
...
class ReviewService:
    ...
    def _validate_semantic_review_details(
        ...
        *,
        ...
        if semantic_item_reviews is None and added_semantic_items is None:
            return

        review_view = build_semantic_review_view(classification, comment)
        semantic_items = cast(
        ...
        item_by_id = {str(item["item_id"]): item for item in semantic_items}
        valid_item_ids = set(item_by_id)
        valid_item_ids.update(
            f"unexplained-{index}"
            for index, _fragment in enumerate(unexplained_fragments)
        )
        taxonomy = result_taxonomy(connection, result_version_id)
        valid_label_codes = (
            {label.code for label in taxonomy.labels} if taxonomy else set()
        )
        ...
        for added in added_semantic_items or []:
            ...
            if not ReviewService._evidence_exists_in_comment(evidence_text, comment):
    ...
    def _build_derived_result_content(
        ...
        for row in base_units:
            ...
            validated = ValidatedClassification.model_validate(
                {
                    field: value
                    ...
                    not in (
                        *HUMAN_REVIEW_CLASSIFICATION_FIELDS,
                        *DERIVED_CLASSIFICATION_FIELDS,
                    )
                }
            )
            published_classification = validated.model_dump(mode="json")
            published_classification.update(
                {
                    field: deepcopy(classification[field])
