...
- 正式产品名为“用户反馈语义分析智能体”；当前支持 Amazon 退货反馈的语义分类、人工复核与业务洞察，不扩展现有数据来源和业务流程。
- 这是绿色、克制、信息密集的桌面 Web“运行驾驶舱”。
- 系统采用普通用户与管理员权限边界，不另设独立审核员角色；普通用户共享分析任务与人工复核，可以查看并协作修改。
- 同时使用人数预计不超过 5 人；每个用户最多同时运行 3 个任务，其他用户的任务可查看和修改。
- 所有人工修改必须展示操作者、修改时间、原值、新值和修改原因，并用版本校验避免覆盖他人的更新。
- 模型服务连接、模型清单等维护和存储清理仅允许管理员执行；普通用户可在现有权限内使用已发布模型，运行中的任务继续使用启动时的配置快照。
...
- 生产界面的默认名称、占位符和示例不得绑定具体店铺、站点、Listing 或品类数据；业务名称应来自运行数据或由后端统一生成。
- 桌面长流程优先使用完整可用宽度和高度，避免短侧栏长期占列形成大面积空白；高频重复编辑应提供可撤销、需重新确认的批量操作。
