...
def test_scope_difference_has_actionable_diagnostic():
    first = result_with_fact()
    second = result_with_fact(condition="仅首次")

    diagnostic = reconcile_secondary(first, second).review_diagnostics[0]

    assert diagnostic.evidence_text == "1. This worked well."
    assert "业务作用域" in diagnostic.detail
    assert "条件=仅首次" in diagnostic.secondary_result
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("fact_v2", "evidence-fact-validator-v14"),
    ],
)
