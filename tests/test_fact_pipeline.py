...
def test_two_calls_map_only_selected_branch_and_cache(fact_taxonomy, tmp_path):
    ...
    audit = json.loads(client.messages[1][1]["content"])
    second = json.loads(client.messages[2][1]["content"])
    third = json.loads(client.messages[3][1]["content"])
    assert "labels" not in first
    assert audit["existing_facts"] == [fact.model_dump(mode="json")]
    assert {label["code"] for label in second["labels"]} == {"WARM", "COLD"}
    assert {label["code"] for label in third["labels"]} == {"WARM", "COLD"}
    assert "基础操作能够完成只证明功能可用" in client.messages[2][0]["content"]
...
def test_unique_topic_route_is_normalized_to_branch_code(fact_taxonomy):
    fact = make_fact(candidate_branch_codes=["保暖"])
    client = FakeJsonClient(
        [
            {"facts": [fact.model_dump(mode="json")]},
            {"mappings": [{"fact_id": "a", "label_codes": ["WARM"]}]},
    ...
    assert result.classification.extracted_facts[0].candidate_branch_codes == ["功能"]
    assert result.classification.semantic_units[0].label_code == "WARM"
...
def test_mapping_retries_only_failed_stage(fact_taxonomy):
    ...
    assert result.metrics["fact_model_calls"] == 5
    assert result.usage["total_tokens"] == 25
    assert "上次输出未通过校验" in client.messages[3][-1]["content"]
...
def test_mapping_model_cannot_overwrite_program_audit_fields(fact_taxonomy):
    ...
    mapping_payload = json.loads(client.messages[2][1]["content"])
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "category-fact-v2-v31",
        "category-fact-v2-v32",
    ],
)
def test_fact_prompt_update_invalidates_previous_fact_cache(
    ...
    assert prompt.prompt_version(fact_taxonomy) == "category-fact-v2-v33"
...
def test_fact_prompts_define_participant_certainty_and_entailment_contracts(
    ...
    assert "如not bulky或not heavy" in extraction_instruction
    assert "不得把topic或标签名当作code" in extraction_instruction
    assert "逐句检查独立属性、性能、尺码与部位" in coverage_instruction
    assert "未实际购买、穿戴或测试的备选规格" in coverage_instruction
    assert "并原样输出该分支的code" in coverage_instruction
    assert "evidence_relation必须按逻辑支持关系填写" in mapping_instruction
    assert "fallback_is_independent=true" in mapping_instruction
    assert "基础能力EVIDENCE必须label_codes为空" in mapping_instruction
    assert "商品细节、结构或描述差异不等于发错商品" in mapping_instruction
    assert "不得推断做工或质量问题" in mapping_instruction
    assert "证据明确不蕴含候选标签时ABSTAIN" in adjudication_instruction
    assert "基础能力EVIDENCE必须ABSTAIN" in adjudication_instruction
    assert "不得裁决为做工或质量标签" in adjudication_instruction
    assert (
        "不得在原文未明示商品身份、规格或颜色错误时裁决为发错商品"
