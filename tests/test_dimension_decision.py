...
def test_fact_pipeline_calls_decision_stage_and_accumulates_usage() -> None:
    fact = _fact("F1", "Typing is inaccurate.", "NEGATIVE")
    client = _FakeJsonClient(
        [
            {"facts": [fact.model_dump(mode="json")]},
            {"mappings": [{"fact_id": "F1", "label_codes": ["TOUCH_NEGATIVE"]}]},
            {
                "decisions": [
                    _decision("TOUCHSCREEN", "TOUCH_NEGATIVE", ["F1"]).model_dump(
                        mode="json"
                    )
                ]
            },
    ...
    instruction = client.messages[4][0]["content"]
